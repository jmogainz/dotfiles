require 'qf'.setup {
  -- Location list configuration
    ...
    c = {
        ...
        auto_open = true, -- Automatically open list on QuickFixCmdPost
        auto_resize = true, -- Auto resize and shrink location list if less than `max_height`
        max_height = 16, -- Maximum height of location/quickfix list
        min_height = 5, -- Minimum height of location/quickfix list
        wide = true, -- Open list at the very bottom of the screen, stretching the whole width.
...
local qf = require 'qf'
-------------------------------------------------
-- 1. Save every time the quickfix/location list
--    is (re)populated by a :make/:grep/… command
-------------------------------------------------
vim.api.nvim_create_autocmd('QuickFixCmdPost', {
  callback = function()
    -- 'c' → quickfix, 'l' → location.  Call both if you like.
    pcall(qf.save, 'c')  -- use default key (command name)
    pcall(qf.save, 'l')
  end,
})

-------------------------------------------------
-- 2. Optionally save again when leaving Neovim
-------------------------------------------------
...
-------------------------------------------------
-- 3. Load the last‑saved list on startup
-------------------------------------------------
vim.api.nvim_create_autocmd('VimEnter', {
  once = true,
  callback = function()
    -- suppress errors if nothing was saved yet
    pcall(qf.load, 'c')
    pcall(qf.load, 'l')
  end,
})
